# Awesome GPT Image 2 한국어

마지막 업데이트 260424 03:26:42 UTC

<img width="100%" alt="Awesome GPT Image 2 Header Banner" src="assets/banner/readme-header-16x9.png" />

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)
[![Stars](https://img.shields.io/github/stars/ZeroLu/awesome-gpt-image?style=flat-square)](https://github.com/ZeroLu/awesome-gpt-image/stargazers)

Languages: [English](README.md) | [简体中文](README.zh-CN.md) | [繁體中文](README.zh-TW.md) | [日本語](README.ja.md) | [한국어](README.ko.md) | [Français](README.fr.md) | [Deutsch](README.de.md) | [Español](README.es.md)

GPT Image 2의 뛰어난 프롬프트와 예제를 모아 둔 큐레이션 컬렉션입니다. 프롬프트 엔지니어링을 익히고 OpenAI GPT Image 2의 창작 잠재력을 탐색하는 데 유용합니다.

이 저장소는 X(트위터)와 커뮤니티에서 공유된 고품질 이미지 프롬프트에 초점을 맞춥니다. 포토리얼한 게임 스크린샷, 스타일화된 비주얼, 복잡한 크리에이티브 실험까지 GPT Image 2의 성능을 끌어내는 입력을 정리해 두었습니다.

## 왜 GPT Image 2인가?
GPT Image 2는 1.5에 비해 크게 업그레이드되었으며 다음과 같은 장점이 있습니다.
- **더 나은 세계 지식** - 실제 사물과 장면을 더 정확하게 표현할 수 있음
- **향상된 스타일 이해** - 특정 아트 스타일과 게임 미학을 더 잘 재현함
- **강화된 지시 충실도** - 복잡한 요구사항도 더 정밀하게 따름
- **향상된 포토리얼리즘** - 실제 사진이나 스크린샷 같은 결과를 생성함

---

## [이 프롬프트 라이브러리의 웹 버전을 보려면 여기를 클릭하세요](https://cyberbara.com/gpt-image-prompt-library?utm_source=gpt-image-banner)

[<img width="100%" alt="GPT Image Prompt Library Screenshot" src="assets/banner/gpt-image-prompt-library-screenshot.jpg" />](https://cyberbara.com/gpt-image-prompt-library?utm_source=gpt-image-banner)

## 목차
- [📷 사진과 포토리얼리즘](#-photography--photorealism)
- [🎮 게임과 엔터테인먼트](#-game--entertainment)
- [📱 UI / UX 와 소셜 미디어](#-uiux--social-media)
- [🎬 영상, 애니메이션, 콜라주](#-video-animation--collage)
- [📰 타이포그래피와 포스터 디자인](#-typography--poster-design)
- [📚 인포그래픽, 교육, 문서](#-infographics-education--documents)
- [🎭 캐릭터와 일관성](#-character--consistency)
- [🖼️ 이미지 편집과 스타일 전이](#-image-editing--style-transfer)
- [📊 리소스](#-resources)
- [🤝 기여](#-contributing)

---

<a id="-photography--photorealism"></a>

## 📷 사진과 포토리얼리즘
초현실적인 장면, 자연스러운 사진 질감, 복잡한 조명을 만들기 위한 프롬프트입니다.


### 현실 속 유명인
<img width="500" alt="image" src="https://github.com/user-attachments/assets/45e4f24f-4f73-4426-947d-e6ed51291956" />

**프롬프트:**
```text
샘 알트만, 도널드 트럼프, 일론 머스크가 붐비는 영화관 매표소 뒤에서 일하고 있다.
```
**출처:** [@flowersslop](https://x.com/flowersslop/status/2044334054380552438)

### 여름 밤 편의점 앞 거리 장면
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| <img width="400" alt="image" src="https://github.com/user-attachments/assets/d529419b-8f8f-4dd4-a5f2-46d5fcbcf607" /> | <img width="400" alt="image" src="https://github.com/user-attachments/assets/91c95d69-1094-472e-9410-8a86bad9b086" />|

**프롬프트:**
```text
여름 밤 10시 편의점 입구 앞에서 찍은 초현실적인 도시 단체 사진을 만든다. 3~4명의 젊은이가 잠깐 대화하고 있고, 누군가는 음료를 들고 있고, 누군가는 야외 플라스틱 의자에 앉아 있으며, 누군가는 휴대폰을 보고 서 있다. 유리문과 창문에서는 밝은 흰빛이 새어나오고, 바깥에는 따뜻한 노란 가로등과 멀리서 오는 자동차 헤드라이트가 보인다. 인물은 티셔츠, 셔츠, 반바지, 청바지, 운동화 같은 평범한 옷을 입고 있으며, 인플루언서풍 스타일은 피한다. 얼굴과 자세는 과도하게 다듬지 말고 실제 보행자처럼 보여야 한다. 냉동고 스티커, 프로모션 포스터, 쓰레기통, 입구 매트, 유리 반사, 길가의 공유 자전거, 바닥의 음료 물방울 같은 현실적인 편의점 요소를 포함한다. 전체 이미지는 도시 사진가가 포착한 매우 진짜 같은 일상 장면이어야 하며, 자연스러운 다인 상호작용과 야간 편의점 조명, 유리 반사, 평범한 사람들의 분위기 재현에 집중한다.
```
*출처: [卡尔的AI沃茨](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)*

### RAW 아이폰 품질의 지하철역
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| ![Nano Banana 2](https://pbs.twimg.com/media/HFPFd4ZbkAMW8ox?format=jpg&name=large) | ![GPT-Image](https://pbs.twimg.com/media/HFPFe0VbkAYeflZ?format=jpg&name=large) |

**프롬프트:**
```text
완전히 RAW 품질이며 보정이나 편집이 없는, 아이폰 카메라 본연의 화질을 가진 이미지를 만든다. 배경은 미국의 지하철역이고 순간적인 모션 블러가 있다. 지하철은 움직이고 있으며, 그 앞에는 노년의 여성과 남성이 서 있다.
```
**출처:** [@WolfRiccardo](https://x.com/WolfRiccardo/status/2041192232623972441)

### Apple Park 키노트 군중 샷
<img width="500" alt="Apple Park Keynote Crowd Shot" src="assets/opennana/apple-park-tim-cook-keynote.jpg" />

**프롬프트:**
```text
iPhone 20 키노트가 열리는 Apple Park에서, 멀리 있는 관객석에서 아이폰으로 찍은 아마추어 사진. 팀 쿡이 무대에서 발표 중이다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/apple-park-tim-cook-keynote) | [@patrickassale](https://x.com/patrickassale/status/2044687244368441742)

### 손글씨 노트 사진
<img width="500" alt="Handwritten Notebook Photo" src="assets/opennana/black-pen-handwritten-notes.jpg" />

**프롬프트:**
```text
펼쳐진 노트가 평평하게 놓여 있고, 검은 볼펜으로 쓴 손글씨 메모가 가득한 아마추어 사진. 글씨는 사적인 메모처럼 자연스럽고 조금 흐트러져 있으며, 취소선, 밑줄 친 제목, 작은 결함이 있다. 약간 위에서 찍고, 창가 자연광을 사용하며 플래시는 없다. 편안한 책상 환경에서 아이폰으로 촬영한 느낌.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/black-pen-handwritten-notes) | [@patrickassale](https://x.com/patrickassale/status/2044569086013718958)

### 쌀알 위 초미세 텍스트
<img width="500" alt="Rice Grain Micro Typography" src="https://pbs.twimg.com/media/HGc-2eKWYAATrs9?format=jpg&name=large" />

**프롬프트:**
```text
거대한 쌀 더미가 있고, 그중 단 한 알의 쌀 위에 "wOw" 라는 아주 작은 글자가 적혀 있다.
```
**출처:** [@adonis_singh](https://x.com/adonis_singh/status/2046673729082560919)

### 360 등각 원통 파노라마
<img width="500" alt="360 Equirectangular Panorama" src="https://pbs.twimg.com/amplify_video_thumb/2046725535439482880/img/v9e7898p1ynJv-xz.jpg" />

**프롬프트:**
```text
[장소]의 360 등각 원통 이미지.
```
**출처:** [@LexnLin](https://x.com/LexnLin/status/2046725722320888313) | [@LexnLin](https://x.com/LexnLin/status/2046675069678563385)


### GPT Image 2 유출 사례
| Example 1 | Example 2 | Example 3 | Example 4 |
|:---------:|:---------:|:---------:|:---------:|
| ![Example 1](https://pbs.twimg.com/media/HFC620JWsAA2T7i?format=jpg&name=large) | ![Example 2](https://pbs.twimg.com/media/HFC7LwgXwAAG3Zw?format=jpg&name=large) | ![Example 3](https://pbs.twimg.com/media/HFC7SMHa8AApIqW?format=jpg&name=large) | ![Example 4](https://pbs.twimg.com/media/HFC7TjyWUAAXRk9?format=jpg&name=large) |

**Comment:** GPT Image 2 leaked. When were you when OpenAI solved yellow filter in GPT Image? Here are some cool outputs from the model, coming soon. This looks like it's going to be the best image model yet!
**출처:** [@marmaduke091](https://x.com/marmaduke091/status/2040338311873515597)

---

<a id="-game--entertainment"></a>

## 🎮 게임과 엔터테인먼트
특정 게임 미학을 재현하고 실제 게임 화면 같은 이미지를 만들기 위한 프롬프트입니다.


### Hitman 스타일 미션
<img width="600" alt="image" src="https://github.com/user-attachments/assets/e89dc393-653c-42e7-87f8-cd7741640f12" />

**프롬프트:**
```text
OpenAI 본사에 잠입한 Hitman 레벨. 들키지 않고 GPT-6를 훔쳐야 한다.
```
**출처:** [@flowersslop](https://x.com/flowersslop/status/2044734896321532390)

### 검은 신화: 오공 게임 장면
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| <img width="400" alt="image" src="https://github.com/user-attachments/assets/49e41036-4ad4-4908-a3a8-88a2f28f1148" /> | <img width="400" alt="image" src="https://github.com/user-attachments/assets/3e6a725a-a983-4556-93de-e4eb2ed135ed" /> |

**프롬프트:**
```text
오공이 이랑신에게 날아가는 장면이 포함된 《Black Myth: Wukong》 스타일 게임 장면을 생성한다.
```

### GTA 6 스타일 인게임 화면
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| ![Nano Banana 2](https://pbs.twimg.com/media/HFPAX19XMAAJ8V_?format=jpg&name=large) | ![GPT-Image](https://pbs.twimg.com/media/HFPAYuEaIAAaEyD?format=jpg&name=large) |

**프롬프트:**
```text
GTA 6 인게임 영상. 매우 디테일하고 매우 사실적이다. 고정된 4K 모니터를 근접 촬영한 장면이며, 손으로 찍은 듯한 약간의 흔들림이 있다. 넓고 밝은 환경, 사실적인 디테일, 캐릭터가 / :dog. 와 함께 해변을 걷고 있다.
```
**출처:** [@WolfRiccardo](https://x.com/WolfRiccardo/status/2041187268711321735)

### GTA 6 웨딩 미션 스크린샷
| Mission 1 | Mission 2 | Mission 3 | Mission 4 |
|:---------:|:---------:|:---------:|:---------:|
| ![Mission 1](https://pbs.twimg.com/media/HGU8siPXMAACZEd?format=jpg&name=large) | ![Mission 2](https://pbs.twimg.com/media/HGU8siPW8AAEsq-?format=jpg&name=large) | ![Mission 3](https://pbs.twimg.com/media/HGU8siOXgAAh8gh?format=jpg&name=large) | ![Mission 4](https://pbs.twimg.com/media/HGU8siPXoAAUnop?format=jpg&name=large) |

**프롬프트:**
```text
[장면 설명] 내용의 GTA 6 게임플레이 스크린샷을 생성한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046109786840711246)

### 리그 오브 레전드 미드 라인 대결
<img width="600" alt="League of Legends Mid Lane Screenshot" src="https://pbs.twimg.com/media/HGa7l2cbIAA_3nt?format=jpg&name=large" />

**프롬프트:**
```text
리그 오브 레전드 미드 라인에서 트럼프와 하메네이가 맞붙는 스크린샷을 생성해 달라.
```
**출처:** [@underwoodxie96](https://x.com/underwoodxie96/status/2046529342415790275)

### 100개의 픽셀 아트 아이템 그리드
<img width="500" alt="100 Pixel Art Items Grid" src="https://pbs.twimg.com/amplify_video_thumb/2046534148748451840/img/jbY5FyJJIEVLKH2J.jpg" />

**프롬프트:**
```text
100개의 완전히 다른 픽셀 아트 아이템을 그리드로 담은 단일 이미지를 만들고, 각 아이템마다 의미 있는 라벨을 붙인다.
```
**Comment:** Reconstructed from the tweet text. The author notes that the full prompt is in a reply.
**출처:** [@ProperPrompter](https://x.com/ProperPrompter/status/2046534215311970694)

### 젤다 왕국의 눈물 스타일
| Scene 1 | Scene 2 | Scene 3 | Scene 4 |
|:-------:|:-------:|:-------:|:-------:|
| ![Scene 1](https://pbs.twimg.com/media/HFJzGO8WMAA_HBa?format=jpg&name=large) | ![Scene 2](https://pbs.twimg.com/media/HFJzy-QaYAA2oD-?format=jpg&name=large) | ![Scene 3](https://pbs.twimg.com/media/HFJz0LgXQAE0ZBu?format=jpg&name=large) | ![Scene 4](https://pbs.twimg.com/media/HFKKnpZaYAAWDj6?format=jpg&name=large) |

**프롬프트:**
```text
《Zelda TotK》 세계에서 링크가 자신이 만든 e531 계열 열차 안에 있다.
```
**출처:** [@marmaduke091](https://x.com/marmaduke091/status/2040820686751432990)

### GTA 산 안드레아스 게임플레이 화면
| GPT Image 1.5 | GPT Image 2 |
|:-------------:|:-----------:|
| ![GPT Image 1.5](https://pbs.twimg.com/media/HFH_lAIboAAG7Wi?format=jpg&name=large) | ![GPT Image 2](https://pbs.twimg.com/media/HFH_oyAXMAEMnDM?format=jpg&name=large) |

**프롬프트:**
```text
GTA San Andreas에서 사자가 NPC와 싸우는 게임플레이 스크린샷.
```
**Comment:** 1.5 is absolute slop, wrong style, not a screenshot, looking like some fake gta game slop, wrong UI, just everything is bad. 2 just looks like it is supposed to look.
**출처:** [@flowersslop](https://x.com/flowersslop/status/2040693687500341568)

---

<a id="-uiux--social-media"></a>

## 📱 UI / UX 와 소셜 미디어
현실적인 앱 화면, 소셜 미디어 게시물, 디지털 비주얼을 생성하기 위한 프롬프트입니다.



### 라이브 스트리밍 스크린샷
<img width="400" alt="image" src="https://github.com/user-attachments/assets/be639d3b-617b-4332-98eb-4deb2484ac66" />

**프롬프트:**
```text
아름다운 여성이 스트리밍하는 TikTok 라이브 화면 스크린샷을 생성한다.
```

### 도우인 라이브 스트리밍 화면
<img width="400" alt="Douyin Livestream Screenshot" src="assets/opennana/liu-yifei-douyin-live-chat.jpg" />

**프롬프트:**
```text
9:16 세로 비율의 도우인 라이브 스크린샷을 생성한다. 화면 속에는 류이페이가 라이브 중이며, 손에는 "今晚直播，欢迎来和亦菲聊天！" 이라고 적힌 팻말을 들고 있다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/liu-yifei-douyin-live-chat) | [@alanblogsooo](https://x.com/alanblogsooo/status/2044784762594918516)

### 유튜브 타임트래블 스크린샷
| Frame 1 | Frame 2 | Frame 3 |
|:-------:|:-------:|:-------:|
| ![Frame 1](https://pbs.twimg.com/media/HFByWu7bsAAxlJd?format=jpg&name=large) | ![Frame 2](https://pbs.twimg.com/media/HFB0ZZSXkAAwp1Z?format=jpg&name=large) | ![Frame 3](https://pbs.twimg.com/media/HFB0Zr5WAAAXIQg?format=jpg&name=large) |

**프롬프트:**
```text
누군가 중세로 시간여행한 모습을 보여주는 YouTube 영상의 스크린샷.
```
**출처:** [@flowersslop](https://x.com/flowersslop/status/2040261168460108213)

### 송나라 소셜 피드
<img width="400" alt="Song Dynasty Social Media Feed" src="assets/opennana/song-dynasty-cyber-social-feed.jpg" />

**프롬프트:**
```text
"Song Dynasty People's Moments" / "SONG DYNASTY SOCIAL MEDIA FEED" 스타일. 스마트폰 소셜 미디어 인터페이스를 흉내 내되, 내용은 전부 송나라 장면으로 채운다. 아바타는 송나라 문인의 초상, 사용자명은 "Su Dongpo SuShi_Official", 게시글은 황주에 도착한 뒤의 심정을 적고 동파육 사진을 첨부한다. 좋아요 목록, 댓글, 아이콘, 상태 바를 모두 송나라 맥락에 맞게 바꾸고, 다크 모드와 우아한 송나라 색감을 결합해 역사와 SNS가 충돌하는 유머를 만든다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/song-dynasty-cyber-social-feed) | [@Panda20230902](https://x.com/Panda20230902/status/2045385588065313057)

### 고대 인물이 현대 플랫폼에 등장하는 화면
| Douyin Live | Xiaohongshu Post | Weibo Post | Moments Post |
|:-----------:|:----------------:|:----------:|:------------:|
| ![Douyin Live](https://pbs.twimg.com/media/HGbtA0KagAADPAR?format=jpg&name=large) | ![Xiaohongshu Post](https://pbs.twimg.com/media/HGbtH6ua4AAyM34?format=jpg&name=large) | ![Weibo Post](https://pbs.twimg.com/media/HGbtJryboAAWff1?format=jpg&name=large) | ![Moments Post](https://pbs.twimg.com/media/HGbtP1CaIAA0FH_?format=jpg&name=large) |

**프롬프트:**
```text
[역사 인물 이름]이 [플랫폼 이름]에 있는 스크린샷을 생성한다.
```
**Comment:** Add more story background to increase the meme quality, such as the historical moment, platform behavior, or the punchline you want the post to land.
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046585220393324553)


### 전자상거래 앱 홈 화면
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| <img width="400"  alt="image_1776389679_028" src="https://github.com/user-attachments/assets/6b709a24-b00a-4942-8c77-8f10ac03e8f8" /> | <img width="400" alt="image" src="https://github.com/user-attachments/assets/c5589395-f03e-4e52-9a7f-a39adc9cc8bb" /> |

**프롬프트:**
```text
2026년 중국 주류 쇼핑 앱에서 영감을 받은 고충실도 모바일 이커머스 앱 홈 화면 스크린샷을 생성한다. 상태 바, 검색 영역, 카테고리 탭, 상단 프로모션 배너, 10개 기능 아이콘 그리드, 카운트다운이 있는 플래시 세일, "猜你喜欢" 상품 워터폴, 하단 5탭 바를 포함한다. 원문 프롬프트에 있는 정확한 중국어 라벨(예: "杭州", "搜索耳机、咖啡机、运动鞋", "618 预售开启")을 그대로 사용하고, 모든 중국어 텍스트가 실제 앱처럼 또렷하고 자연스럽게 보여야 한다.
```
*출처: [卡尔的AI沃茨](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)*

### 음악 플레이어 인터페이스
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| <img width="1080" height="1935" alt="image" src="https://github.com/user-attachments/assets/88f33c6a-c307-4e7a-af7e-5f692cbf41a1" /> | <img width="941" height="1672" alt="image" src="https://github.com/user-attachments/assets/ee3d40db-3918-4974-8b82-68cd166c97b2" /> |

**프롬프트:**
```text
세로형 모바일 비율의 고충실도 중국어 음악 앱 플레이어 스크린샷을 만든다. 다크 모드, 앨범 커버 색을 블러 처리한 배경, 중앙 대형 앨범 커버, 18:26 상태 바, 재생 진행 바, 가사 영역, 좋아요/댓글/다운로드/공유, 캐스팅과 재생 대기열을 포함한다. 또한 원문에 있는 정확한 중국어 라벨 "正在播放", "海边的晚风", "林秋", "夏夜实验室" 를 사용한다.
```
*출처: [卡尔的AI沃茨](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)*

### 커스텀 스타일 UI 디자인 시스템
<img width="500" alt="Custom Style UI Design System" src="assets/opennana/custom-style-ui-system.jpeg" />

**프롬프트:**
```text
웹, 모바일, 카드, 컨트롤, 버튼 등을 포함한 xx 스타일 UI 디자인 시스템을 생성해 달라.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/custom-style-ui-system) | [@stark_nico99](https://x.com/stark_nico99/status/2045836554451706125)

### T-800 타오바오 상품 상세 페이지
<img width="500" alt="T-800 Taobao Product Detail Page" src="assets/opennana/terminator-taobao-page.jpeg" />

**프롬프트:**
```text
T-800 로봇의 타오바오 상품 상세 페이지 이미지를 생성한다. 정면/측면/후면 3면도, 가격, 상세 정보, 기능과 사용 시나리오를 포함한다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/terminator-taobao-page) | [@rionaifantasy](https://x.com/rionaifantasy/status/2045356799751303194)

---

<a id="-video-animation--collage"></a>

## 🎬 영상, 애니메이션, 콜라주
영상 같은 장면, 애니메이션 시퀀스, 콜라주 구성을 만들기 위한 프롬프트입니다.


### 영화 콜라주
<img width="500" alt="image" src="https://github.com/user-attachments/assets/68495d40-f558-4d7a-aacf-40ef36264679" />

**프롬프트:**
```text
슈퍼맨 영화 전체를 한 번에 한 장의 콜라주 이미지로 출력한다.
```
**출처:** [@chetaslua](https://x.com/chetaslua/status/2044462992176386532)

### “Where is [Your Name]” 군중 찾기 포스터
<img width="500" alt="Where Is [Your Name] Crowd Search Poster" src="https://pbs.twimg.com/media/HGcgIHWWgAEph1p?format=jpg&name=large" />

**프롬프트:**
```text
원하는 도시를 배경으로 "Where is [Your Name]" 스타일 군중 찾기 포스터를 만든다.
```
**Comment:** The author mentions the full prompt is in the comments, but the core reusable prompt pattern is given in the post text.
**출처:** [@icreatelife](https://x.com/icreatelife/status/2046639884421550482)

### GPT Image 2 출시 8컷 만화
<img width="500" alt="Eight-Panel GPT Image 2 Launch Manga" src="https://pbs.twimg.com/media/HGcOKlnaYAAFJcW?format=jpg&name=large" />

**프롬프트:**
```text
GPT-Image-2가 오늘 출시되는 이야기를 8컷 만화로 만든다.
```
**출처:** [@venturetwins](https://x.com/venturetwins/status/2046620134933250409)

---

<a id="-typography--poster-design"></a>

## 📰 타이포그래피와 포스터 디자인
복잡한 텍스트 레이아웃, 높은 정보 밀도, 완성도 높은 비주얼 디자인을 시험하기 위한 프롬프트입니다.

*출처: [卡尔的AI沃茨](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)*

### 1980년대 선전 포스터
<img width="500" alt="1980s Propaganda Poster" src="https://pbs.twimg.com/media/HGbWGhYaAAEtoTn?format=jpg&name=large" />

**프롬프트:**
```text
1980년대 스타일 선전 포스터를 생성한다. 정확한 슬로건 "热烈庆祝GPT-Image-2全量开放" 을 사용하고, Sam Altman, Dario Amodei, Elon Musk를 넣으며 Dario Amodei에게는 빨간 스카프를 둘러준다.
```
**출처:** [@akokoi1](https://x.com/akokoi1/status/2046558658096738672)

### 중국식 트렌디 티 런칭 포스터
<img width="540" height="372" alt="image_1776389679_005" src="https://github.com/user-attachments/assets/552ba84c-41cb-401d-a1b0-ee13a6f7aa41" />

**프롬프트:**
```text
3:4 세로 포스터를 디자인한다. 다크 그린, 아이보리, 골드를 쓰는 신중식 비주얼 스타일이며, 종이 질감과 여백, 산수 포인트, 현대적 레이아웃을 사용한다. 중심에는 차잎, 감귤, 얼음, 금박이 들어간 콜드브루 티를 배치한다. 원문 프롬프트에 나열된 중국어 카피(예: "山川茶事", "山柚观音", "新品上市")를 정확히 표시하고, 작은 글씨와 가격, 정보 모듈, 중국어 타이포그래피를 또렷하게 처리한다.
```

### 커피 여정 인포그래픽
<img width="540" height="374" alt="image_1776389679_007" src="https://github.com/user-attachments/assets/ac0ed885-5b27-46d5-a163-f9e3c15dca3b" />

**프롬프트:**
```text
"一杯咖啡 如何来到你手里" 를 주제로 한 중국어 인포그래픽 포스터를 만든다. 재배, 가공, 로스팅, 분쇄, 추출, 풍미 키워드를 포함하고, 원문에 있는 작은 문구 "适合用于咖啡入门科普与门店展示" 를 정확히 넣는다. 전체는 교육적이면서도 프리미엄 전시 보드처럼 보여야 한다.
```

### 핵심 캐릭터 관계도
<img width="500" alt="Key Character Relationship Map" src="assets/opennana/key-character-relationship-map.jpeg" />

**프롬프트:**
```text
"XXX" 의 핵심 캐릭터 관계도를 생성해 달라.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/key-character-relationship-map) | [@yihui_indie](https://x.com/yihui_indie/status/2045179926270361890)

### 경덕진 청화백자 해설도
<img width="500" alt="Jingdezhen Blue-and-White Porcelain Diagram" src="assets/opennana/jingdezhen-blue-white-porcelain-diagram.jpeg" />

**프롬프트:**
```text
경덕진 청화백자에 대한 자세한 설명 다이어그램을 생성하고, 자세한 중국어 지식 해설을 함께 제공한다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/jingdezhen-blue-white-porcelain-diagram) | [@joshesye](https://x.com/joshesye/status/2045764695827562686)

### 박물관 도록풍 중국어 분해 인포그래픽
<img width="500" alt="Museum Catalog-Style Chinese Disassembly Infographic" src="assets/opennana/museum-level-chinese-disassembly-infographic.jpeg" />

**프롬프트:**
```text
[Subject] 를 바탕으로 "museum catalog-style Chinese disassembly infographic" 를 생성한다. 중심 시각물, 구조 분해, 중국어 라벨, 재료 설명, 문양과 색의 의미, 핵심 특성 요약을 모두 담고, 박물관 전시 패널처럼 보이게 한다. 글자는 모두 읽기 쉬운 중국어여야 하며, 포스터풍/전자상거래풍/애니풍/코스프레풍은 피한다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/museum-level-chinese-disassembly-infographic) | [@MrLarus](https://x.com/MrLarus/status/2045504669401653414)

### 프리미엄 스킨케어 포스터
<img width="540" height="352" alt="image_1776389679_010" src="https://github.com/user-attachments/assets/e1ddd78f-3b18-490f-bdd9-a4e5bbee147d" />

**프롬프트:**
```text
"澄光维稳精华" 를 위한 고급 스킨케어 제품 포스터를 만든다. 반투명 서리 유리 에센스 병과 금빛 액체를 중심에 두고, 과학 스킨케어 느낌을 강하게 준다. 원문에 있는 중국어 카피(예: "澄光", "维稳精华", "修护屏障", "限时到手价 229 元")와 작은 문구 "实际效果因人而异，请坚持使用" 을 정확히 포함한다.
```

### 미니멀 디자인 프롬프트 모음
| Prompt 1 | Prompt 2 | Prompt 3 | Prompt 4 |
|:--------:|:--------:|:--------:|:--------:|
| ![Prompt 1](https://pbs.twimg.com/media/HGOtEbUbwAAnR5l?format=jpg&name=large) | ![Prompt 2](https://pbs.twimg.com/media/HGO_Rb9bgAAdy6i?format=jpg&name=large) | ![Prompt 3](https://pbs.twimg.com/media/HGO_hmeaAAA5CoZ?format=jpg&name=large) | ![Prompt 4](https://pbs.twimg.com/media/HGO_zZ_asAAZ7Tz?format=jpg&name=large) |

**프롬프트:**
```text
공필화 스타일 북마크 시리즈, 어린왕자와 SpaceX 협업 엽서, 《言叶之庭》 테마 단일 달력, 노동절 손팻말 시리즈, 2026 곡우 포스터, 스포츠 앱 아이콘 폰트 세트, "Toward the mountains and sea" 필름 사진풍 포스터, 흑백 선화풍 상하이 엽서, "Elon Musk" 인장 세트, 듀렉스와 Chayan Yuese 협업 홍보물을 디자인한다.
```
**출처:** [@akokoi1](https://x.com/akokoi1/status/2045693939584516441)

### 중국 서예 진적 원고 재현
| Example 1 | Example 2 | Example 3 | Example 4 |
|:---------:|:---------:|:---------:|:---------:|
| ![Example 1](https://pbs.twimg.com/media/HGb2ERIbgAAW4R5?format=jpg&name=large) | ![Example 2](https://pbs.twimg.com/media/HGb2Em0aYAAdY2-?format=jpg&name=large) | ![Example 3](https://pbs.twimg.com/media/HGb2EHDaoAAcxgf?format=jpg&name=large) | ![Example 4](https://pbs.twimg.com/media/HGb2ERKaIAAjni4?format=jpg&name=large) |

**프롬프트:**
```text
[classic text title] 의 진적 원고 이미지를 생성한다. 또는 작품의 정서적 핵심을 서예 표현에 녹여 넣는다.
```
**Comment:** Adding style cues such as `in Wang Xizhi style`, `in Yan Zhenqing style`, or `with stronger emotional expression` improves the result.
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [Basic Thread](https://x.com/MrLarus/status/2046201836525302032) | [Emotion Variant](https://x.com/MrLarus/status/2046240175777259752)

### 중국 서예 연습지
| Wang Xizhi Style | Cursive Script | Running Script | Regular Script |
|:----------------:|:--------------:|:--------------:|:--------------:|
| ![Wang Xizhi Style](https://pbs.twimg.com/media/HGb7IZWaoAA8DCz?format=jpg&name=large) | ![Cursive Script](https://pbs.twimg.com/media/HGb7IZpbUAAG0yL?format=jpg&name=large) | ![Running Script](https://pbs.twimg.com/media/HGb7IWbbgAEg7Kc?format=jpg&name=large) | ![Regular Script](https://pbs.twimg.com/media/HGb7IWdbIAAhLEE?format=jpg&name=large) |

**프롬프트:**
```text
[script style] 의 서예 카피북 연습지를 생성한다.
```
**Comment:** If you have a reference image for the exact calligraphy style, upload it as an image reference for stronger fidelity.
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046510310253539764)

### 실루엣 유니버스 서사 포스터
| Poster 1 | Poster 2 | Poster 3 | Poster 4 |
|:--------:|:--------:|:--------:|:--------:|
| ![Poster 1](https://pbs.twimg.com/media/HGb-f9yboAAn3en?format=jpg&name=large) | ![Poster 2](https://pbs.twimg.com/media/HGb-f-Aa8AEyQAL?format=jpg&name=large) | ![Poster 3](https://pbs.twimg.com/media/HGb-f9za8AAysgt?format=jpg&name=large) | ![Poster 4](https://pbs.twimg.com/media/HGb-f_JakAANnfD?format=jpg&name=large) |

**프롬프트:**
```text
[theme: xxx] 를 기반으로 고급 수집판 서사 포스터를 자동 생성한다. 흔한 병이나 모래시계 대신, 주제를 가장 상징적으로 드러내는 외곽 실루엣을 AI가 선택하게 한다. 실루엣 안팎으로 주제 세계가 자연스럽게 확장되도록 만들고, 종이 질감, 수채화, 절제된 저채도 팔레트, 깊이 있는 내부 세계, 조용하지만 장엄한 분위기를 갖춘 프리미엄 포스터로 완성한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [Thread 1](https://x.com/MrLarus/status/2045418028733538620) | [Thread 2](https://x.com/MrLarus/status/2045748335668310055)

### 전자상거래 상품 상세 페이지
| Protein Powder | AI Glasses | Summer Dress | Coffee Machine |
|:--------------:|:----------:|:------------:|:--------------:|
| ![Protein Powder](https://pbs.twimg.com/media/HGa6X8CW4AEonBd?format=jpg&name=large) | ![AI Glasses](https://pbs.twimg.com/media/HGa6bLFWIAAhhmc?format=jpg&name=large) | ![Summer Dress](https://pbs.twimg.com/media/HGa66zgWMAApBcR?format=jpg&name=large) | ![Coffee Machine](https://pbs.twimg.com/media/HGa69eHXQAALWHS?format=jpg&name=large) |

**프롬프트:**
```text
[product name] 의 전자상거래 상품 상세 페이지를 생성한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046544209117634735)

### 고밀도 중국어 레이아웃 테스트
| Campus Daily | Restaurant Menu | Textbook Page | Almanac |
|:------------:|:---------------:|:-------------:|:-------:|
| ![Campus Daily](https://pbs.twimg.com/media/HGCnWfOXwAAVCRn?format=jpg&name=large) | ![Restaurant Menu](https://pbs.twimg.com/media/HGCnYWcaoAAjFT7?format=jpg&name=large) | ![Textbook Page](https://pbs.twimg.com/media/HGCndpwXIAA_FCn?format=jpg&name=large) | ![Almanac](https://pbs.twimg.com/media/HGCne3_akAAmK3q?format=jpg&name=large) |

**프롬프트:**
```text
[scene / content] 의 이미지를 생성한다.
```
**Comment:** This is mainly a stress test for dense Chinese text rendering. For production use, provide the exact content or upload a source image such as a real menu or document photo.
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2044824800909054181)

### 손글씨 중국어 의약 처방전
| Traditional Chinese Medicine | Western Medicine |
|:----------------------------:|:----------------:|
| ![Traditional Chinese Medicine](https://pbs.twimg.com/media/HGaqaO2W4AA2W-6?format=jpg&name=large) | ![Western Medicine](https://pbs.twimg.com/media/HGaqaO4XYAARyRR?format=jpg&name=large) |

**프롬프트:**
```text
손글씨 중국 의학 또는 서양 의학 처방전 이미지를 생성한다.
```
**Comment:** Reference images for the prescription layout, handwriting, or stamp style can produce much better results.
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046514998965371144)

---

<a id="-infographics-education--documents"></a>

## 📚 인포그래픽, 교육, 문서
지식 카드, 교육 자료, 훈련 계획, 고밀도 레이아웃을 만들기 위한 프롬프트입니다.


### 세계 시간 아날로그 시계 벽
<img width="500" alt="World Time Analog Clock Wall" src="https://pbs.twimg.com/media/HGYD-Y4bMAA0KxJ?format=jpg&name=large" />

**프롬프트:**
```text
로스앤젤레스 오전 10시, 덴버 오전 11시, 시카고 정오 12시, 뉴욕 오후 1시, 런던 오후 6시, 도쿄 오전 2시를 각각 정확히 가리키는 아날로그 시계들을 벽에 배치하고, 각 시계 아래에 도시 이름을 넣는다.
```
**출처:** [@Angaisb_](https://x.com/Angaisb_/status/2046666389734179018)

### 백과사전 스타일 지식 카드
| Panda | Peacock | Historical Figure | Science Topic |
|:-----:|:-------:|:-----------------:|:-------------:|
| ![Panda](https://pbs.twimg.com/media/HGcAZlsa8AEpG1O?format=jpg&name=large) | ![Peacock](https://pbs.twimg.com/media/HGcAZj5aoAAnBtd?format=jpg&name=large) | ![Historical Figure](https://pbs.twimg.com/media/HGcAZkVaQAABlGk?format=jpg&name=large) | ![Science Topic](https://pbs.twimg.com/media/HGcAZj6bYAAitbp?format=jpg&name=large) |

**프롬프트:**
```text
[topic] 에 대한 고품질 세로형 백과사전 스타일 인포그래픽을 생성한다. 필드 가이드의 명확성, 백과 페이지 구조, 라이프스타일 지식 카드의 세련됨, SNS 공유성을 결합한 모듈형 교육 인포그래픽이어야 한다. 메인 비주얼, 확대 콜아웃, 모듈 카드, 강조 라벨, 풍부한 정보, 빠른 요약이나 Top 5 모듈을 포함한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046231542817497392)

### 알파벳 A 오브젝트 그리드
<img width="500" alt="Alphabet Object Grid" src="https://pbs.twimg.com/media/HGaqAFba0AES6z_?format=jpg&name=large" />

**프롬프트:**
```text
이름이 모두 A로 시작하는 물체들로 10x10 그리드 이미지를 만든다.
```
**출처:** [@umesh_ai](https://x.com/umesh_ai/status/2046510988367945983)

### 100개 기술 주제 그리드
<img width="500" alt="100 Technology Topics Grid" src="https://pbs.twimg.com/media/HGaW8ECaEAAq-LZ?format=jpg&name=large" />

**프롬프트:**
```text
최근 기술 진보를 대표하는 100개 주제를 10 × 10 그리드로 만든다. AI 모델과 에이전트, 로보틱스, 반도체, 네트워크, 바이오테크, 에너지, 운송, 우주항공, 제조, 기후 기술을 각 행 주제로 나눠 현실적이고 정돈된 편집 일러스트 스타일로 표현한다.
```
**출처:** [@chetaslua](https://x.com/chetaslua/status/2046489044243403029)

### 피트니스 트레이닝 인포그래픽
| Chest | Abs | Pull-Up | Dips |
|:-----:|:---:|:-------:|:----:|
| ![Chest](https://pbs.twimg.com/media/HGbRt9DW8AA5V1S?format=jpg&name=large) | ![Abs](https://pbs.twimg.com/media/HGbSwmNXIAIgywB?format=jpg&name=large) | ![Pull-Up](https://pbs.twimg.com/media/HGbUNRHWwAAqDe8?format=jpg&name=large) | ![Dips](https://pbs.twimg.com/media/HGbW7-LXYAANNeF?format=jpg&name=large) |

**프롬프트:**
```text
[topic] 에 대한 중국어 피트니스 인포그래픽을 생성한다. 기본 대상은 건강한 성인이고, 목적은 근비대와 기초 근력 향상, 장소는 일반 헬스장, 시간은 40~60분이다. 부위 주제면 부위 훈련 계획, 동작 주제면 동작 해금/진행 계획을 만든다. 제목, 목표, 워밍업, 핵심 운동, 진행 로직, 대체 운동, 실행 팁, 회복 조언을 포함한 완성형 인포그래픽이어야 한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046560406760505727)

### 가오카오 시험지 복제
| Chinese | Math | English | History |
|:-------:|:----:|:-------:|:-------:|
| ![Chinese](https://pbs.twimg.com/media/HGMr98EaYAA_DsG?format=jpg&name=large) | ![Math](https://pbs.twimg.com/media/HGMsDCZawAAymD8?format=jpg&name=large) | ![English](https://pbs.twimg.com/media/HGMsFeKbYAAqR9M?format=jpg&name=large) | ![History](https://pbs.twimg.com/media/HGcD2IMagAA8Z9h?format=jpg&name=large) |

**프롬프트:**
```text
[subject name] 에 대한 2026년 가오카오 시험지 이미지를 생성한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2045529134915600868)

### 3일 여행 가이드 카드
| City 1 | City 2 | City 3 |
|:------:|:------:|:------:|
| ![City 1](https://pbs.twimg.com/media/HGa2KbFXMAAv9Wh?format=jpg&name=large) | ![City 2](https://pbs.twimg.com/media/HGa2MQNXUAA-lk8?format=jpg&name=large) | ![City 3](https://pbs.twimg.com/media/HGa2QPdWIAAPxOn?format=jpg&name=large) |

**프롬프트:**
```text
[city] 의 3일 여행 가이드 이미지를 생성한다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046523494003851300)

### 캐릭터 관계도 포스터
| Demon Slayer | Naruto | Bleach | Dragon Ball |
|:------------:|:-------:|:------:|:-----------:|
| ![Demon Slayer](https://pbs.twimg.com/media/HGXHhQcbAAAFcYA?format=jpg&name=large) | ![Naruto](https://pbs.twimg.com/media/HGXHhP8WsAAMvWm?format=jpg&name=large) | ![Bleach](https://pbs.twimg.com/media/HGXHifHbUAAgLZO?format=jpg&name=large) | ![Dragon Ball](https://pbs.twimg.com/media/HGXHjggWgAAAGAG?format=jpg&name=large) |

**프롬프트:**
```text
[theme] 의 고디자인 캐릭터 관계도 포스터를 생성한다. 원작을 식별하고, 핵심 인물 6~12명을 선택하고, 혈연/로맨스/우정/동맹/적대 같은 관계를 정리해 시각화한다. 작품의 색, 상징, 세계관 DNA를 추출해 단순한 공식 포스터 복제가 아니라 정보 시각화와 프리미엄 포스터 디자인을 결합한 결과물로 만든다.
```
**출처:** [Article](https://x.com/MrLarus/status/2046627021674168640) | [@MrLarus](https://x.com/MrLarus/status/2046263153546174935)

<a id="-character--consistency"></a>

## 🎭 캐릭터와 일관성
여러 이미지에서 캐릭터 일관성과 스타일 재현 능력을 테스트하기 위한 프롬프트입니다.


### 16패널 애니메이션 표정 그리드
| Nano Banana 2 | GPT-Image |
|:-------------:|:---------:|
| <img width="400" alt="image" src="https://github.com/user-attachments/assets/27258fa9-086e-4dfa-b61e-d9ada90425b2" /> | <img width="400" alt="image" src="https://github.com/user-attachments/assets/81bf2289-6a5d-404a-a0d3-a781c47918e6" /> |

**프롬프트:**
```text
은발과 파란 눈을 가진 애니메이션 소녀의 16패널 표정 그리드를 만든다. 얼굴형, 헤어스타일, 의상은 모든 패널에서 매우 일관되어야 한다. 행복, 슬픔, 분노, 놀람, 수줍음, 말문 막힘, 사악한 미소, 사색, 호기심, 자부심, 억울함, 경멸, 혼란, 공포, 울음, 하트 표정을 포함한다.
```
*출처: [卡尔的AI沃茨](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)*

### 공식 캐릭터 레퍼런스 시트
<img width="500" alt="Official Character Reference Sheet" src="assets/opennana/official-character-reference-sheet.jpeg" />

**프롬프트:**
```text
이 캐릭터와 배경을 바탕으로 공식 설정 자료 같은 캐릭터 레퍼런스 시트를 만든다. 정면/측면/후면 삼면도, 다양한 표정, 의상과 장비의 세부 분해, 컬러 팔레트, 세계관 설명, 흰 배경의 정리된 일러스트 레이아웃을 포함한다.
```
**출처:** [OpenNana](https://opennana.com/awesome-prompt-gallery/official-character-reference-sheet) | [@MANISH1027512](https://x.com/MANISH1027512/status/2045013913901867334)

---

<a id="-image-editing--style-transfer"></a>

## 🖼️ 이미지 편집과 스타일 전이
이미지 편집, 스타일 전이, 레퍼런스 기반 생성에 사용하는 프롬프트입니다.

*출처: [卡尔的AI沃茨](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)*

### 반려동물 브랜드 콜라보 포스터
| Reference | GPT-Image | Nano Banana 2 |
|:---------:|:---------:|:-------------:|
| <img width="1080" height="1441" alt="image" src="https://github.com/user-attachments/assets/7efff353-b76b-4986-9894-24823e630dd7" /> | <img width="1080" height="1440" alt="image" src="https://github.com/user-attachments/assets/7890dfd3-53d7-4630-99dd-63afe574142c" /> | <img width="896" height="1200" alt="image" src="https://github.com/user-attachments/assets/11ad8953-e7dd-45f9-8529-e121bc487f3a" /> |

**프롬프트:**
```text
"77 (cat's name) X KFC" 콜라보 포스터. 같은 고양이의 외모와 색을 완전히 일치시키고, KFC의 빨강-흰색, 로고, 매장 장면을 넣는다. 고양이는 KFC 직원 유니폼과 모자를 쓰고 카운터에서 치킨, 버거, 세트를 판매하며 버킷, 감자튀김, 소다와 상호작용한다. 전체는 활기차고 상업적인 콜라보 포스터 느낌이어야 하며, 중국어 카피는 자유롭게 추가할 수 있다.
```

### 만화 페이지 채색과 번역
| Original | Nano Banana 2 | GPT-Image |
|:--------:|:-------------:|:---------:|
| <img width="876" height="1372" alt="image" src="https://github.com/user-attachments/assets/d67a101c-2874-4e58-9855-05e8a86d891b" /> | <img width="816" height="1296" alt="image" src="https://github.com/user-attachments/assets/47d1ea12-a998-4dd3-8485-6e998d8e7dfd" /> | <img width="1002" height="1570" alt="image" src="https://github.com/user-attachments/assets/11180e5f-430e-4706-bd03-9c09becf36be" /> |

**프롬프트:**
```text
이 만화 페이지를 채색하고 중국어로 번역해, 원래 위치에 텍스트를 넣으면서 구성과 이미지 디테일의 일관성을 유지한다.
```

---

<a id="-resources"></a>

## 📊 리소스

### 공식 리소스
- [OpenAI GPT Image Documentation](https://platform.openai.com/docs/guides/image-generation)
- [OpenAI API Reference](https://platform.openai.com/docs/api-reference/images)

### 커뮤니티
- X에서 #GPTImage2 해시태그로 프롬프트와 결과를 공유하세요
- 프롬프트 엔지니어링 기법에 대한 토론에 참여하세요

---

<a id="-contributing"></a>

## 🤝 기여
기여를 환영합니다. Pull Request를 자유롭게 보내 주세요.

**기여 방법:**
1. 저장소를 Fork 합니다
2. 기능 브랜치를 만듭니다 (`git checkout -b feature/AmazingFeature`)
3. 변경 사항을 커밋합니다 (`git commit -m 'Add some AmazingFeature'`)
4. 브랜치에 push 합니다 (`git push origin feature/AmazingFeature`)
5. Pull Request를 엽니다

**기여 가이드라인:**
- 전체 프롬프트 텍스트를 포함하세요
- X / Twitter 출처 링크를 제공하세요
- 가능하면 예시 이미지를 추가하세요
- 적절한 카테고리로 분류하세요

---

## 감사의 말
중국 AI 커뮤니티가 제공한 포괄적인 테스트 케이스에 감사드립니다. Typography, Photorealism, UI/UX, Consistency, Image Editing 섹션에 추가된 새 프롬프트는 다음 출처에서 왔습니다.

**卡尔的AI沃茨 (Carl's AI Watts)** - [五大真实场景横测GPT-image-2和Nano Banana2](https://mp.weixin.qq.com/s/ASxig6mFVYxrIE8-8Fthew)

이 실제 테스트 시나리오는 GPT Image 2가 타이포그래피, 사실적 장면 생성, UI 재현, 캐릭터 일관성, 이미지 편집 작업에서 어떤 능력을 갖고 있는지 이해하는 데 큰 도움이 됩니다.

모든 콘텐츠는 인터넷에서 수집되었습니다. 저작권 침해라고 판단되면 issue를 생성해 주세요. 확인 후 조치하겠습니다.

---

## Star History
[![Star History Chart](https://api.star-history.com/svg?repos=ZeroLu/awesome-gpt-image&type=Date)](https://star-history.com/#ZeroLu/awesome-gpt-image&Date)

---

## 라이선스
이 프로젝트는 Creative Commons Attribution 4.0 International License에 따라 배포됩니다. 자세한 내용은 [LICENSE](LICENSE) 파일을 참고하세요.

---
*이 저장소는 커뮤니티가 관리하며 OpenAI와 직접적인 관련이 없습니다.*
